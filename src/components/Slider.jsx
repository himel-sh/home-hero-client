...
const Slider = () => {
  ...
  return (
    <div className="relative w-full h-[250px] lg:h-[600px] mx-auto overflow-hidden shadow-lg">
      <AnimatePresence initial={false} custom={direction}>
        <motion.img
          ...
          onDragEnd={(e, { offset, velocity }) => {
            const swipePower = offset.x / window.innerWidth;
            if (swipePower < -0.25 || velocity.x < -200) paginate(1);
            else if (swipePower > 0.25 || velocity.x > 200) paginate(-1);
          }}
      ...
      <button
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/70 text-black p-3 rounded-full shadow-lg hover:bg-white transition z-30"
        onClick={() => paginate(-1)}
      >
        ❮
      </button>
      <button
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/70 text-black p-3 rounded-full shadow-lg hover:bg-white transition z-30"
        onClick={() => paginate(1)}
      >
        ❯
      </button>
